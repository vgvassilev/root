...
std::string cached_realpath(llvm::StringRef path, llvm::StringRef base_path = "",
  ...
  // If already cached - retun cached result
  static llvm::StringMap<llvm::StringRef> cache;
  ...
  if (llvm::sys::path::is_relative(path)) {
    if (is_base_path_real) {
      ...
      if (base_path.empty()) {
        static std::string crp;
        if (crp.empty()) {
          llvm::SmallString<256> current_path;
          llvm::sys::fs::current_path(current_path);
          crp = cached_realpath(path, current_path);
        }
        llvm::StringRef(crp).split(p, sep, /*MaxSplit*/ -1, /*KeepEmpty*/ false);
      } else {
